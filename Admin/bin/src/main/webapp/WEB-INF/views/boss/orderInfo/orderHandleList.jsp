...
<body style="font-family: '微软雅黑'">
		...
		<div id="tb" style="padding:5px;height:auto">
			<div>
				<form id="searchFrom" action="">
					<input type="hidden" name="filter_EQI_status" value="6" class="easyui-validatebox" data-options="width:60,prompt: '采购商id'" />
					<!-- 		采购商查询 start	 -->
					...
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false"
						onclick="handle2Fail()">手工失败</a> <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true"
						data-options="disabled:false" onclick="handle2Success()">手工成功</a> 
						<a href="javascript:void(0)" class="easyui-linkbutton"
						iconCls="" plain="true" data-options="disabled:false" onclick="handle2Back()">手工回调</a> 
						<a href="javascript:void(0)"
						class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false" onclick="handle2Send()">手工重发</a> 
						<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false"
						onclick="getOrderStatus()">查询广东订单状态</a>
						
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false" onclick="changeOrder()">改变订单号</a> 
						<br />
					<div style="height: 7px;"></div>
				</form>
				<div style="height: 7px;"></div>
	...
	<script type="text/javascript">
		...
		function handle2Fail() {
			...
			var color = "red";
			var title = '已选<span style="color:' + 'green' + ';font-weight:bold;">'+rows.length+'</span>条记录,您确定要<span style="color:red;font-weight:bold">手工失败</span>?';
		...
		function handle2Success() {
			...
			var color = "green";
			var title = '已选<span style="color:' + 'green' + ';font-weight:bold;">'+rows.length+'</span>条记录,您确定要<span style="color:green;font-weight:bold">手工成功</span>?';
			parent.$.messager.confirm('提示', title, function(data) {
				if (data) {
		...
		function handle2Back() {
			...
			var color = "green";
			var title = '已选<span style="color:' + 'green' + ';font-weight:bold;">'+rows.length+'</span>条记录,您确定要<span style="color:green;font-weight:bold">手工回调</span>?';
			parent.$.messager.confirm('提示', title, function(data) {
				if (data) {
					var ids = new Array();
					for (var i = 0; i < rows.length; i++) {
		...
		function getOrderStatus() {
			...
			var color = "green";
			var title = '已选<span style="color:' + 'green' + ';font-weight:bold;">'+rows.length+'</span>条记录,您确定要<span style="color:green;font-weight:bold">查询广东订单状态</span>?';
			parent.$.messager.confirm('提示', title, function(data) {
				if (data) {
					var ids = new Array();
					for (var i = 0; i < rows.length; i++) {
		...
		function handle2Send() {
			...
			var color = "green";
			var title = '已选<span style="color:' + 'green' + ';font-weight:bold;">'+rows.length+'</span>条记录,您确定要<span style="color:green;font-weight:bold">手工重发</span>?';
			parent.$.messager.confirm('提示', title, function(data) {
		...
		function changeOrder(){
			...
			$.ajax({ 
				url : "${ctx}/boss/orderInfo/changeOrder", 
				type : "post", 
				traditional : true,
				data : { ids : ids}, 
				success : function(data) {
