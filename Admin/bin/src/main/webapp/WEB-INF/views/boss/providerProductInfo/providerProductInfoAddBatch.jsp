...
<body>
	<div>
		<form id="mainform" action="${ctx}/boss/providerProductInfo/addBatch" method="post">
			<table class="formTable">
				...
				<tr>
					<td>产品类别信息：</td>
					<td><select id="categoryId1" name="categoryId" multiple="multiple"  size="20" onclick="check()" onchange="getProductName()"
						onblur="getCategoryIds()">
							<option value="">请选择</option>
					</select> <input name="categoryIds" type="hidden" /></td>
				</tr>
				<tr>
					<td>状态：</td>
				...
				<tr>
					<td>通道优先级：</td>
					<td><input name="priority" type="text" value="1" class="easyui-validatebox" /></td>
				</tr>
				<tr>
					<td>二次通道：</td>
	...
	<script type="text/javascript">
		...
		function getCategoryIds() {
			var returnVal = "";
			$("select[name=categoryId] option:selected").each(function() { //遍历全部option
				returnVal+=","+$(this).val();
			});
			if(returnVal!=""){
				returnVal = returnVal.substring(1,returnVal.length);
				$("input[name=categoryIds]").val(returnVal);
			}
		}
		//设置标准价的格式
