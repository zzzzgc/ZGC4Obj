...
<body>
<div>
	<form id="mainform" action="${ctx}/boss/providerBalanceRecord/${action}" method="post">
	<table  class="formTable">
		...
		<tr>
			<td>供应商：</td>
			<td>
				<input id="customerName" name="providerName" value="${balance.providerId}" data-options="width: 180;"  readonly="readonly"/>
			</td>
		</tr>
		<tr>
			<td>注资类型：</td>
		...
		<tr>
			<td>是否通过：</td>
			<td>
				<input type="radio" name="status" value="3" class="easyui-validatebox" data-options="validType:'requireRadio[\'input[name=status]\', \'通过  或者 不通过 \']'"><span id="pass">通过</span> <input type="radio" name="status" value="2"><span id="noPass">不通过</span>
			</td>
		</tr>
		<tr>
			<td>备注：</td>
...
function getCustomerName(value){
	var customerName=${applicationScope.sysParam.providerInfoMap};
	var retrunVal="";
