	...
	<div id="tb" style="padding:5px;height:auto">
		<div>
			<form id="searchFrom" action="">
				<input type="text" name="filter_GED_addTime" value="" class="easyui-my97" startDate='%y-%M-%d 00:00:00'
					datefmt="yyyy-MM-dd HH:mm:ss" data-options="width:150,prompt: '添加时间'" />
				--
				<input type="text" name="filter_LED_addTime" value="" class="easyui-my97" startDate='%y-%M-%d 23:59:59'
					datefmt="yyyy-MM-dd HH:mm:ss" data-options="width:150,prompt: '添加时间'" />
				<input type="text" name="filter_GED_updateTime" value="" class="easyui-my97" startDate='%y-%M-%d 00:00:00'
					datefmt="yyyy-MM-dd HH:mm:ss" data-options="width:150,prompt: '更新时间'" />
				--
				<input type="text" name="filter_LED_updateTime" value="" class="easyui-my97" startDate='%y-%M-%d 23:59:59'
					datefmt="yyyy-MM-dd HH:mm:ss" data-options="width:150,prompt: '更新时间'" />
				...
				</select> <span class="toolbar-item dialog-tool-separator"></span> 是否限价：<select name="productCategoryInfo_isLimit">
					<option value="">请选择</option>
					<c:forEach items="${applicationScope.sysParam.isNoMap}" var="isNoMap">
						<option value="${isNoMap.key}">${isNoMap.value}</option>
					</c:forEach>
				...
				<shiro:hasPermission name="boss:providerProductInfo:changeDiscount">
				...
				<span class="toolbar-item dialog-tool-separator"></span> &nbsp;
				<input type="reset" value="更改优先级" onclick="updatePrior()" />
			...
			<shiro:hasPermission name="boss:providerProductInfo:delete">
				...
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" data-options="disabled:false" onclick="delAll()">批量删除</a>
				<span class="toolbar-item dialog-tool-separator"></span>
			</shiro:hasPermission>
			...
			<shiro:hasPermission name="boss:providerProductInfo:showPriceAndDis">
...
// 													}
											...
											{ field : 'costPrice', title : '成本价',hidden:mark,
												formatter : function(value, row, index) {
		...
		function updateStatus(html, state) {
			...
			parent.$.messager.confirm('提示', '已选<span style="color:' + color + ';font-weight:bold;">' + rows.length
					...
					function(data) {
						if (data) {
							$.ajax({ url : "${ctx}/boss/providerProductInfo/updateStatus", type : "post",
								traditional : true, data : { "ids" : ids, "state" : state }, success : function(value) {
									dg.datagrid('reload');
								}
			...
			d = $("#dlg").dialog(
				{
					...
					width : 380,
					height : 250,
					href : '${ctx}/boss/providerProductInfo/updateDataList',
					maximizable : true,
					modal : true,
					buttons : [
							{
								...
								handler : function() {
									...
									var ids = [];
									var discount = $("#discount1").val();
									var title = '您的数据为<span style="color:green;font-weight:bold">'+ discount + '</span>';
									
										if (parseFloat(discount) > 1) {
											title = '您的数据为<span style="color:red;font-weight:bold;font-size:30px;">'
													+ discount + '</span>';
										}
											...
											function(data) {
												if (data) {
													...
													$.ajax({
																traditional : true,
																type : "post",
																url : "${ctx}/boss/providerProductInfo/updatePrior",
																data : { "ids" : ids,"status" : discount},
																success : function(data) {
																	if (data == "success") {
																		d.panel('close');
																		cx();
					...
					function(data) {
						if (data) {
							...
							$.ajax({
										traditional : true,
										type : "post",
										url : "${ctx}/boss/providerProductInfo/delAll",
										data : { "ids" : ids},
										success : function(data) {
											if (data == "success") {
												parent.$.messager.show({title : "提示",msg : "操作成功！",position : "bottomRight" });
											}else{
												parent.$.messager.alert("删除错误");
											}
												cx();
												$('#dg').datagrid('clearSelections');
										} });
