			...
			<div>
				<form id="searchFrom" action="">
					...
					<input type="text" name="filter_EQS_providerKey" class="easyui-validatebox" data-options="width:80,prompt: '供应商流水号'"
						onchange="trimLeft(this)" />
					<input type="text" name="filter_EQS_phone" class="easyui-validatebox" data-options="width:120,prompt: '手机号码'"
						onchange="trimLeft(this)" />
					原始状态：<select name="filter_EQI_status">
						<option value="">请选择</option>
						...
						<!--  -->
					最终状态：<select name="filter_EQI_finalStatus">
						<option value="">请选择</option>
						<c:forEach items="${applicationScope.sysParam.orderStatusMap}" var="orderStatusMap">
							<option value="${orderStatusMap.key}">${orderStatusMap.value}</option>
						</c:forEach>
					...
					<div style="display:inline;margin-left: 200px">
						<input type="button" value="修改订单为手工处理状态" onclick="callback(this)" />
					</div>
						<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="add('bossOrderResendRecord/create');">添加</a>
...
<!-- 					data-options="disabled:false" onclick="exportExcel()">导出Excel</a> -->
	...
	<div id="bottom" style="height:5%;width:100%; position:absolute;bottom:0;">
		<div style="display:-moz-box;display:-webkit-box;display:box;width:100%;">
			<div>
				<button onclick="getCountInfo()">统计信息</button>
			</div>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			dg = $('#dg').datagrid(
					{
						method : "POST",
						url : '${ctx}/boss/bossOrderResendRecord/bossOrderResendRecordList',
						fit : true,
						autoRowHeight : false,
						fitColumns : true,
						queryParams : { 
...
// 							filter_LED_receiveTime : $("input[name='filter_LED_receiveTime']").val(), 
						...
						columns : [
							[
									...
									{ field : 'status', title : '原始状态', formatter : function(value, row, index) {
										if (value == 1) {
											return "新增";
										} else if (value == 2) {
											return "充值中";
									...
									{ field : 'finalStatus', title : '最终状态', formatter : function(value, row, index) {
										if (value == 1) {
											return "新增";
										} else if (value == 2) {
											return "充值中";
		...
		function add() {
			d = $("#dlg").dialog(
					{ title : '添加配置', width : 380, height : 250, href : '${ctx}/boss/bossOrderResendRecord/create',
		...
		function getCountInfo() {
			...
			var obj = $("#searchFrom").serializeObject();
			$.ajax({
				url : "${ctx}/boss/bossOrderResendRecord/getCountInfo",
				type : "get",
				data : obj,
...
// 			filter_LED_receiveTime.val(getNowFormatDate(date));
		...
		function exportExcel() {
			$.messager.confirm('提示', '确定要导出excel？', function(data) {
				if (data) {
					...
					var url = "${ctx}/boss/bossOrderResendRecord/exportExcel" + params;
			...
			parent.$.messager.confirm('提示', '您确定要<span style="color:green;font-weight:bold;">' + title + '</span>吗?',
					function(data) {
						if (data) {
							...
							$.ajax({ url : "${ctx}/boss/bossOrderResendRecord/updateOrderStatus", type : "post",
		...
		function getBalckList() {
			$.ajax({ url : "${ctx}/boss/bossOrderResendRecord/getBlackList", type : "get", success : function(data) {
