...
import xinxing.boss.admin.common.utils.reflection.Reflections;
...
public class HibernateDao<T, PK extends Serializable> extends BaseHibernateDao<T, PK> {
	 ...
	 * 按属性过滤条件列表分页查找对象.
	 */
	public Page<T> findPage(final Page<T> page, final List<PropertyFilter> filters,Integer maxNumber) {
		Criterion[] criterions = buildCriterionByPropertyFilter(filters);
		return findPage(page,maxNumber, criterions);
	}
	/**
	 * 按HQL分页查询.
	 * 
	...
	public Page<T> findPage(final Page<T> page, final Criterion... criterions) {
		...
		List result = c.list();
//		log.info("HibernateDAO  result====="+result.size());
		page.setResult(result);
		return page;
	}
	/**
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Page<T> findPage(final Page<T> page,Integer maxNumber, final Criterion... criterions) {
		Assert.notNull(page, "page不能为空");
...
//		log.info("HibernateDAO  result====="+result.size());
	 ...
	 * 按属性条件参数创建Criterion,辅助函数.
	 */
	protected Criterion buildCriterion(final String propertyName, final List propertyValue, final MatchType matchType) {
		Assert.hasText(propertyName, "propertyName不能为空");
		Criterion criterion = null;
			//根据MatchType构造criterion
			switch (matchType) {
			case EQ:
				criterion = Restrictions.eq(propertyName, propertyValue.get(0));
				break;
			case LIKE:
				criterion = Restrictions.like(propertyName, (String) propertyValue.get(0), MatchMode.ANYWHERE);
				break;
			case LE:
				criterion = Restrictions.le(propertyName, propertyValue.get(0));
				break;
			case LT:
				criterion = Restrictions.lt(propertyName, propertyValue.get(0));
				break;
			case GE:
				criterion = Restrictions.ge(propertyName, propertyValue.get(0));
				break;
			case GT:
				criterion = Restrictions.gt(propertyName, propertyValue.get(0));
				break;
			case IN:
	...
	*//**
	...
	public Integer saveOrUpdate(String sql) {
		int executeUpdate = createSQLQuery(sql).executeUpdate();
		 return executeUpdate;
	}
	public Integer saveOrUpdateByHql(String hql,Object...objects) {
	    Query createQuery = createQuery(hql);
