...
<script>
	...
	//省市的二级联动
	function chooseCity() {
		var $province = $("select[name=province] option:selected").val()==undefined?$("select[name=filter_EQS_province] option:selected").val():$("select[name=province] option:selected").val();
				...
				{ title : '添加配置', width : width, height : height, href : '${ctx}/boss/' + url, maximizable : true,
					modal : true, buttons : [
							{ text : '确认', handler : function() {
								console.debug("------");
							console.debug($("#dlg"));
							console.debug($("#dlg .l-btn-text")[0]);
	...
	function getCategoryId() {
		...
		var provinceValue = $("select[name=province] option:selected").val();
		...
		if (categoryName.length == 0 && area.length == 0 && productStandard.length == 0 && operatorValue.length == 0
		...
		if (!(area.length == 0 && categoryName.length == 0 && productStandard.length == 0 && operatorValue.length == 0 && provinceValue.length == 0)
				&& filter_INI_categoryId.val().length == 0) {
			console.debug(area.length);
			console.debug(categoryName.length);
			console.debug(productStandard.length);
			console.debug(operatorValue.length);
			console.debug(provinceValue.length);
			console.debug(filter_INI_categoryId.val().length);
