...
//	public static Integer STATUS_AUDIT = 7;
	...
	public static Integer HANDLE_FAIL = 1; // 失败转手工
	public static Integer HANDLE_HUMEN = 2; // 人工转手工
	public static Integer HANDLE_PROVIDER_EXEPTION = 3; // 供应商回应异常转手工
	public static Integer HANDLE_NO_ORDER = 4; // 无订单手工
	public static Integer HANDLE_OVERTIME = 5; // 超时手工
	public static Integer HANDLE_OPERATOR_CITY_CLOSE = 6; // 运营商地市关闭转手工
	public static Integer HANDLE_NOT_FINISH = 7;// 未完成订单转手工
	public static Integer HANDLE_TMALL_CACHE = 7;// 天猫订单缓存转手工
	public enum Status {
		NEW(1), CHARGE(2), SUCCESS(3), FAIL(4), WAIT(5), HANDLE(6), AUDIT(7);
		public int status;
		private Status(int state) {
			this.status = state;
		}
		public static Status getStatus(Integer status) {
			for (Status orderInfoStatus : Status.values()) {
				if (orderInfoStatus.status == status) {
	...
	private String orderKey;// 采购商订单号,要求对方系统唯一 tmall对应 tmall的id
	...
	private Integer bizType;//业务类型(1流量 2话费)
	...
	/**
	 * 1.失败转手工 2.人工转手工 3.供应商回应异常转手工 4.无订单手工 5.超时手工 6.运营商地市关闭转手工 7.未完成订单转手工
	 * 
	 * @param handleStatus
	 */
