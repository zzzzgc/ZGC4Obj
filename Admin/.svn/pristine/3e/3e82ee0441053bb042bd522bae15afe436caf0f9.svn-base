		...
		<div>
			<form id="searchFrom" action="">
				...
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false"
					onclick="getOrderStatus(this)">查询广东移动订单</a>
				<!-- 	 -->
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false"
					onclick="getOrderStatusAll(this)">查询所有订单</a>
				<!-- 	 -->
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false"
					onclick="handle2status(0)">手工失败</a>
				<!-- 	 -->
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false"
					onclick="handle2status(1)">手工成功</a>
				<!-- 	 -->
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false"
					onclick="handle2Back()">手工回调</a>
				<!-- 	 -->
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true" data-options="disabled:false"
					onclick="handle2Send()">手工重发</a>
				<!-- 	 -->
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="" plain="true"
					data-options="disabled:false" onclick="changeOrder()">改变订单号</a>
				<!-- 	 -->
				<!-- 	 -->
			...
			dg = $('#dg').datagrid(
					{
						...
						queryParams : { filter_GED_receiveTime : $("input[name='filter_GED_receiveTime']").val(),
							...
							filter_EQI_status : 6,
							filter_EQI_handleStatus : $("input[name='filter_EQI_handleStatus']").val(), },
						border : false,
						striped : true,
						...
						columns : [
							[
									...
									{ field : 'callbackStatus', title : '回调状态',
										...
										} }, { field : 'id', title : 'id' }, { field : 'categoryId', title : '产品id' },
		...
		function getOrderStatus(value) {
			...
			parent.$.messager.confirm('提示', title, function(data) {
				if (data) {
					...
					$.ajax({ url : "${ctx}/boss/orderInfo/handleOrderTest", type : "post", traditional : true,
		...
		function handle2status(status) {
			...
			parent.$.messager.confirm('提示', title, function(data) {
				if (data) {
					...
					var action = (status == 0 ? 'toFail' : 'toSuccess');
					$.ajax({
						url : "${ctx}/boss/orderInfo/handleOrderTest",
						type : "post",
						traditional : true,
						data : { ids : ids, action : 'toFail' },
						success : function(data) {
							$('#dg').datagrid('clearSelections');
							if (data != "success") {
								d = $("#dlg").dialog(
										{
											...
											width : 380,
											height : 250,
											href : '${ctx}/boss/bossProviderCharge/handleOrderPage?status=' + status
													+ '&ids=' + data, maximizable : true, modal : true, buttons : [
													{ text : '确认', handler : function() {
														$("#mainform").submit();
													} }, { text : '取消', handler : function() {
														d.panel('close');
